/* ═══════════════════════════════════════════════════════════════════
   ...
   Employee Management System — clock-in gate + Manager/Owner EMS
   dashboard (attendance, leave, salary). Gated by CFG.ems_enabled
   (set by platform app). Moved out of src/admin/ems.js -- it's a
   standalone feature used across POS, Workshop, and Admin, not
   admin-exclusive code, matching the same reasoning as the Inventory
   move.

   NOT YET SPLIT internally: this is currently the full, unsplit file
   content (858 lines) relocated as-is. attendance.js/leave.js/salary.js
   is real follow-up work -- attachEMSEvents() alone is an 87-line
   handler mixing all three domains together, the same shape of problem
   admin.js's attachEvents() has at a smaller scale. Named index.js now
   so that later split won't require touching any of this file's 7
   import call sites across pos.js/workshop.js/admin.js/main.js again.
═══════════════════════════════════════════════════════════════════ */
import {
  sb, state, CFG, money,
  _clearSession,
